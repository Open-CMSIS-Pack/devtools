  ...
  for (const auto& selectedContext : m_selectedContexts) {
    ...
    if (selectedContext.empty()) {
      ...
      if (m_toolchains.empty()) {
        ProjMgrLogger::Error("no compiler registered. Add path to compiler ./bin directory with environment variable <name>_TOOLCHAIN_<major>_<minor>_<patch>. <name> is one of AC6, GCC, IAR, CLANG");
        return false;
      }
