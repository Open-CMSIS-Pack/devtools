...
Allows to control the directory structure for generator output files.  As default (when no explicit `generator-dir:` is specified), the **workingDir** of the [generators element](https://open-cmsis-pack.github.io/Open-CMSIS-Pack-Spec/main/html/pdsc_generators_pg.html#element_generator) in the PDSC file is used.  When no **workingDir** is specified an explicit `generator-dir:` in the `*.yml` input files must be specified.

This control can be added various levels of the `*.yml` input files. The following order is used:
...
:----------------------------|--------------|:------------------------------------
`- <generator-id>: <path>`   |  Optional    | Specifies the directory for saving files created by the generator.
